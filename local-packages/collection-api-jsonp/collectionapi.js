...
CollectionAPI._requestListener = function (server, request, response) {
  ...
  self._requestJsonpCallback = self._server._querystring.parse(self._requestUrl.query)['callback'];
...
CollectionAPI._requestListener.prototype._sendResponse = function(statusCode, body) {
  ...
  // add jsonp wrapper if needed
  if(self._requestJsonpCallback !== undefined) {
    self._response.write(self._requestJsonpCallback + '(' + body + ');')
  } else {
